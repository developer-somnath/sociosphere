...
export default function PlanCreate() {
    const { feature_keys } = usePage<PageProps<CreateProps>>().props;
    const { t } = useI18n();
    ...
    const addFeature = () => {
        const [key, label] = availableKeys[0] ?? [null, null];
        if (!key) return;
        form.setData("features", [
            ...form.data.features,
            { feature_key: key, limit_value: "" },
        ]);
    };
